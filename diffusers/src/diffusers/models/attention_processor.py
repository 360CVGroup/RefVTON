...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        inner_dim = key.shape[-1]
        head_dim = inner_dim // attn.heads
        # print(query.shape, key.shape, value.shape, "query.shape, key.shape, value.shape, 11111")
        ...
        # print(query.shape, key.shape, value.shape, "query.shape, key.shape, value.shape, 22222")
        ...
        if image_rotary_emb is not None:
            ...
            # asdf
            # print(query.shape, key.shape, value.shape, "query.shape, key.shape, value.shape, 33333")
            query = apply_rotary_emb(query, image_rotary_emb)
            key = apply_rotary_emb(key, image_rotary_emb)
            # print(query.shape, key.shape, value.shape, "query.shape, key.shape, value.shape, 44444")
        ...
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        hidden_states = hidden_states.to(query.dtype)
        # print(hidden_states.shape, "hidden_states.shape_end")
